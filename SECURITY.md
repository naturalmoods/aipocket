...
| Not knowing what the tool talks to | `aipocket audit` prints every request and host before you run anything — including the nine `aipocket probe` requests, which only run when asked but do send the key, and including an `HTTPS_PROXY` from your environment, which becomes a host that receives every credential. `aipocket probe --dry-run` lists a probe's requests without sending them. A disabled provider is contacted by nothing, not even an explicit `aipocket probe`. |
